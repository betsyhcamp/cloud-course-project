...
def mocked_openai():
    ...
    # Tear down and terminate OpenAI mock server process
...
def start_mock_server(port: int, max_retries: int = 3, retry_delay_seconds: int = 1) -> subprocess.Popen:
    ...
    process = subprocess.Popen(
        [
            sys.executable,  # launch mocked server in same virtual env used in running tests
            str(MOCKED_OPENAI_SERVER_PY_PATH),  # mocked server is a python file in the mocks directory
        ],
